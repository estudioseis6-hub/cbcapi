...
[ ] Crear un Fondo nuevo (Admin) no conecta solo con Balance. `POST /fondos` solo inserta en la tabla `fondos` — no crea ninguna cuenta en Plan de Cuentas ni asigna `cuenta_patrimonial`. Hoy, para que un Fondo nuevo funcione de punta a punta, hace falta un paso manual: crear la cuenta correspondiente en Plan de Cuentas (con el nombre contable real, no el código interno del Fondo — ej. "Saldo en Mercado Pago", no "1BCO$"), y asignarle ese Fondo desde ahí. Sin resolver todavía — no automatizar sin pensarlo bien primero, ya que no hay forma de que el sistema adivine un nombre contable correcto para un Fondo nuevo.
