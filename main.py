...
def _cf_anio_default(mes_cierre):
    ...
    hoy = date.today()
    # si ya pasamos el mes de cierre, el ejercicio en curso cierra el anio que viene
    return hoy.year + 1 if hoy.month > mes_cierre else hoy.year


@app.get("/circuito_f/config")
def cf_get_config():
    conn = get_conn()
    try:
        with conn.cursor() as cur:
...
def cf_resumen(anio: Optional[int] = None):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # rango de fechas del ejercicio para las compras
            ...
            # ventas y sueldos cargados (por mes calendario del ejercicio)
        ...
        # proyeccion al cierre: extrapola el promedio de los meses con datos a 12 meses
