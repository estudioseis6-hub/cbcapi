        ...
        with conn.cursor() as cur:
            ...
            sql = """
                SELECT c.id, c.fecha, t.nombre titular, f.nombre fondo,
                       ...
                       c.confirmado, c.id_operacion, c.id_titular, c.id_cheque_apertura, c.cantidad_moneda, c.id_asiento,
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                ...
                JOIN asientos a ON a.id = al.id_asiento
                JOIN asiento_lineas al2 ON al2.id_asiento = al.id_asiento AND al2.id != al.id
                LEFT JOIN plan_de_cuentas p2 ON p2.id = al2.id_cuenta
                WHERE {" AND ".join(where)}
...
def get_mayor_por_fondo(mes: Optional[int] = None, anio: Optional[int] = None,
    ...
    """Planilla de Ingresos y Egresos agrupada por Cuenta contable, con el total de cada una
    repartido en las mismas 4 columnas de Fondo que ya se usan en Tesorería (EFVO $,
...
def get_balance_unificado(mes: Optional[int] = None, anio: Optional[int] = None,
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT id, niv1, niv2, niv3, niv4, niv5, niv6, niv1_desc, niv2_desc, niv3_desc, niv4_desc, niv5_desc, niv6_desc, nombre, id_codigo, signo
